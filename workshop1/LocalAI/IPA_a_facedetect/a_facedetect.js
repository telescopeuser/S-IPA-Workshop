...
var flow_path = '/home/iss-user/iss-vm-program/is-intelligent-process-automation/TagUI/src/samples-a_facedetect';

casper.start('https://www.iss.nus.edu.sg/about-us/staff/detail/201/GU%20Zhan', function() {
techo('https://www.iss.nus.edu.sg/about-us/staff/detail/201/GU%20Zhan' + ' - ' + this.getTitle() + '\n');});
...
casper.then(function() {for (n=1; n<=number_of_frgnd_images; n++)
...
casper.then(function() {{techo('snap '+current_image+' to '+profile_name+'/'+n+'.png');
casper.waitFor(function check() {return check_tx(''+current_image+'');},
function then() {this.captureSelector(''+abs_file(''+profile_name+'/'+n+'.png')+'',tx(''+current_image+''));},
function timeout() {this.echo('ERROR - cannot find '+current_image+'').exit();});}});
...
casper.then(function() {for (n=1; n<=number_of_bkgnd_images; n++)
{casper.then(function() {for_loop_signal = '[CONTINUE_SIGNAL][n]';});
(function (n) { // start of IIFE pattern
{ // start of code block
...
casper.then(function() {{techo('snap '+current_image+' to '+profile_name+'/'+(n+number_of_frgnd_images)+'.png');
casper.waitFor(function check() {return check_tx(''+current_image+'');},
function then() {this.captureSelector(''+abs_file(''+profile_name+'/'+(n+number_of_frgnd_images)+'.png')+'',tx(''+current_image+''));},
function timeout() {this.echo('ERROR - cannot find '+current_image+'').exit();});}});
...
casper.then(function() {for (n=1; n<=total_number_of_images; n++)
{casper.then(function() {for_loop_signal = '[CONTINUE_SIGNAL][n]';});
(function (n) { // start of IIFE pattern
{ // start of code block
...
casper.then(function() {{techo('py import face_recognition\nimage = face_recognition.load_image_file(image_file)\nface_locations = face_recognition.face_locations(image)\nprint(len(face_locations))');
py_result = ''; if (!py_step('py import face_recognition\nimage = face_recognition.load_image_file(image_file)\nface_locations = face_recognition.face_locations(image)\nprint(len(face_locations))'))
...
casper.then(function() {if ((py_result == 1))
{ // start of code block
...
casper.then(function() {if ((run_result != ""))
{ // start of code block
...
casper.then(function() {if (((fs.exists(target_file))))
{ // start of code block

...
var flow_path = '/home/iss-user/iss-vm-program/is-intelligent-process-automation/TagUI/src/samples-2_twitter';

casper.start('https://twitter.com/sam_gu_zhan', function() {
techo('https://twitter.com/sam_gu_zhan' + ' - ' + this.getTitle() + '\n');});
...
casper.then(function() {{techo('hover ProfileAvatar');
casper.waitFor(function check() {return check_tx('ProfileAvatar');},
function then() {this.mouse.move(tx('ProfileAvatar'));},
function timeout() {this.echo('ERROR - cannot find ProfileAvatar').exit();});}});
...
casper.then(function() {{techo('snap page to profile_image.png');
this.capture('/home/iss-user/iss-vm-program/is-intelligent-process-automation/TagUI/src/samples-2_twitter/profile_image.png');}});

// use read step to fetch webpage element text to variable
casper.then(function() {{techo('read ProfileHeaderCard-nameLink to profile_name');
casper.waitFor(function check() {return check_tx('ProfileHeaderCard-nameLink');},
function then() {profile_name = this.fetchText(tx('ProfileHeaderCard-nameLink')).trim();},
function timeout() {this.echo('ERROR - cannot find ProfileHeaderCard-nameLink').exit();});}});

casper.then(function() {{techo('read ProfileHeaderCard-screennameLink to profile_id');
casper.waitFor(function check() {return check_tx('ProfileHeaderCard-screennameLink');},
function then() {profile_id = this.fetchText(tx('ProfileHeaderCard-screennameLink')).trim();},
function timeout() {this.echo('ERROR - cannot find ProfileHeaderCard-screennameLink').exit();});}});

// use echo step to print text (in quotation marks) and variables to output
casper.then(function() {this.echo("Profile Name - "+profile_name+" (using double quotes for text)");});

// either double or single quotes can be used to denote text from variables
casper.then(function() {this.echo('Profile Name - '+profile_name+' (using single quotes for text)');});

// if you are a JavaScript person, you can also use + or write directly in JS
casper.then(function() {this.echo("Profile Name - "+profile_name+" (using + as separator instead of only space)");});
...
casper.then(function() {this.echo("Profile Name - "+profile_name+", Profile ID - "+profile_id+" (using multiple variables)");});
...
casper.then(function() {{techo('dump "Profile Name - " profile_name ", Profile ID - " profile_id " (using multiple variables)"');
save_text('',"Profile Name - "+profile_name+", Profile ID - "+profile_id+" (using multiple variables)");}});

// data dumps are automatically named textXXXX.txt
// unless you provide a filename for the data dump
casper.then(function() {{techo('dump "Profile Name - " profile_name ", Profile ID - " profile_id " (using multiple variables)" to profile.txt');
save_text('/home/iss-user/iss-vm-program/is-intelligent-process-automation/TagUI/src/samples-2_twitter/profile.txt',"Profile Name - "+profile_name+", Profile ID - "+profile_id+" (using multiple variables)");}});

// use save step if you want to save text from an element to file without further handling
casper.then(function() {{techo('save ProfileHeaderCard-screennameLink');
casper.waitFor(function check() {return check_tx('ProfileHeaderCard-screennameLink');},
function then() {save_text('',this.fetchText(tx('ProfileHeaderCard-screennameLink')).trim());},
function timeout() {this.echo('ERROR - cannot find ProfileHeaderCard-screennameLink').exit();});}});

// saved texts are automatically named textXXXX.txt
// unless you provide a filename for the saved text
casper.then(function() {{techo('save ProfileHeaderCard-screennameLink to profile_id.txt');
casper.waitFor(function check() {return check_tx('ProfileHeaderCard-screennameLink');},
function then() {save_text('/home/iss-user/iss-vm-program/is-intelligent-process-automation/TagUI/src/samples-2_twitter/profile_id.txt',this.fetchText(tx('ProfileHeaderCard-screennameLink')).trim());},
function timeout() {this.echo('ERROR - cannot find ProfileHeaderCard-screennameLink').exit();});}});

// write step appends to file instead of overwriting file (eg dump step)
casper.then(function() {{techo('write \'line 1\' to text_test.txt');
append_text('/home/iss-user/iss-vm-program/is-intelligent-process-automation/TagUI/src/samples-2_twitter/text_test.txt','line 1');}});

casper.then(function() { // start of JS code
line_number = 2
}); // end of JS code

casper.then(function() {{techo('write \'line \' line_number to text_test.txt');
append_text('/home/iss-user/iss-vm-program/is-intelligent-process-automation/TagUI/src/samples-2_twitter/text_test.txt','line '+line_number);}});

// load step loads file content into specified variable
casper.then(function() {{techo('load text_test.txt to file_content');
var fs = require('fs'); file_content = '';
if (fs.exists('/home/iss-user/iss-vm-program/is-intelligent-process-automation/TagUI/src/samples-2_twitter/text_test.txt'))
file_content = fs.read('/home/iss-user/iss-vm-program/is-intelligent-process-automation/TagUI/src/samples-2_twitter/text_test.txt').trim();
else this.echo('ERROR - cannot find file text_test.txt').exit();}});

casper.then(function() {this.echo('showing file content of text_test.txt');});

casper.then(function() {this.echo(file_content);});
